...
"""
        ...
        sshagent(['admin']) {
          sh """
            ssh -o StrictHostKeyChecking=no ${FRONTEND_SERVER} '
              ...
              # ───────────────────────────────────────────────────────────────────────
              # 3) 프론트엔드(nginx) 컨테이너 빌드된 최신 이미지로 기동
              # ───────────────────────────────────────────────────────────────────────
    ...
    stage('7. Cypress E2E 테스트') {
      steps {
        sh '''
          echo "🧼 기존 Cypress 컨테이너 정리"
          docker ps -a --filter ancestor=cypress/included:12.0.0 --format "{{.ID}}" | xargs -r docker rm -f
